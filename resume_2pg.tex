...
\begin{itemize}[itemsep = -0.65 mm, leftmargin=*]
    ...
    \item Utilized \textbf{Haystack} framework and \textbf{FAISS} to efficiently index and store proprietary data, leveraging \textbf{vectorDBs} to store the embeddings. Utilized \textbf{Hugging Face models} to form the \textbf{indexing, retrieval and reranking pipeline} 
...
\vspace{-10pt}
\noindent\textbf{\large Latent Diffusion for Image Generation}\hfill{\sl \small (Summer 2023)}\\
{\it Self Project}
\\\vspace{-15pt}
\begin{itemize}[itemsep = -0.65 mm, leftmargin=*]
    \item Used \textbf{PyTorch} to implement a \textbf{Latent Diffusion} model and evaluate the quality of its generations using \textbf{FID scores}
    ...
    \item Implemented a \textbf{Diffusion U-Net} with \textbf{timestep embeddings} and \textbf{self-attention} and used it to implement both unconditional and conditional \textbf{DDPM (Denoising Diffusion Probabilistic Models)} on the CIFAR-10 dataset 
    \item Implemented \textbf{latent diffusion} by utilizing the diffuser's VAE to \textbf{encode images} to latent representations, and subsequently trained a DDPM on these latents using the LSUN churches datasets to generate high-quality images
\end{itemize}
\vspace{\baselineskip}
\vspace{-15pt}
...
\begin{itemize}[itemsep = -0.65 mm, leftmargin=*]
    ...
    epsilon, gamma, number of timesteps to achieve the best results and solved the environment \textbf{within 500 episodes}
\end{itemize}
...
\vspace{-15pt}
\begin{itemize}[itemsep = -0.65 mm, leftmargin=*]
    \item Developing a messaging software with \textbf{end-to-end encryption} by using \textbf{RSA+AES} to encode the messages and both group chat and individual chat support using \textbf{python socket library, threading} and \textbf{PostgreSQL} database
    \item Implementing a \textbf{load balancer} with \textbf{least connect strategy} using \textbf{bash} to distribute load among multiple servers and also used scripts to simulate common messaging patterns and calculate \textbf{throughput} and \textbf{latency} of the system
\end{itemize}
...
\\\vspace{-15pt}
\begin{itemize}[itemsep = -0.65 mm, leftmargin=*]
    \item Developed a secure web application utilizing \textbf{Node.js, Express.js \& MongoDB} for mimicking the KYC requirements
    \item Learned about \textbf{full-stack development} and utilized \textbf{Bootstrap, EJS, Passport.js} to create a responsive web application with secure authentication, form validation and user sessions. Used \textbf{FastAPI} to make the backend API
...
\vspace{\baselineskip}
\vspace{-13pt}
\noindent\textbf{\large Branch Predictor for Graph Analytics} \hfill{\sl \small (Spring 2023)}\\
{\it Guide: Prof. Biswabandan Panda} $|$ {\it Course Project : Digital Logic Design and Computer Architecture } \hfill{\it IIT Bombay}
\vspace{-2pt}
\begin{itemize}[itemsep = -0.65 mm, leftmargin=*]
    \item Implemented the \textbf{L-TAGE branch predictor} using \textbf{champsim}, a state-of-the-art simulator in \textbf{C++} and analyzed the performance of the predictor on various \textbf{benchmarking traces} like gcc, leslie3d, milc, perlbench and zeusmp
\end{itemize}
\vspace{\baselineskip}
\vspace{-13pt}
\noindent\textbf{\large SwarComm - A Link Layer Simulation} \hfill{\sl \small (Spring 2023)}\\
...
\noindent\begin{tabular}{m{36mm} m{13.7cm}}
    ...
    \hline
    \vspace{3pt} \vspace{-1pt} \textbf{Computer Science} & \vspace{3pt} \vspace{-1pt}  Data Structures and Algorithms, Data Analysis and Interpretation, Software Systems Lab,
    Computer Networks, Computer Architecture, Design and Analysis of Algorithms,
...
\vspace{-7pt}
\begin{itemize}[itemsep = -0.65 mm, leftmargin=*]
    \item Mentored two groups of students during the \textbf{SoC (Summer of Code)} program conducted by \textbf{WNCC, IITB} \\guiding them through \textbf{deep learning projects} and assisting in implementation of cutting-edge research papers  \hfill{\sl \small (2023)}
    ...
    \item Pitched a \textbf{Business Model Canvas} for a startup in the health sector which entailed making online ambu-\\lance
